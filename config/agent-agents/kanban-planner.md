...
- **User explicitly named a project** ("what's next on business-scout?") → override.
...
Default ordering: **In Progress → Up Next → Blocked → Backlog**. Skip Done unless asked. Within each column: priority order (`#p0` first, then `#p1`, `#p2`, untagged).

For each card show:
- ID + title (one line)
- Status badges: column / priority / stage tags (`#bench`/`#pilot`/`#fleet` for driver-shield)
- Due date if `@{date}` present
- Scratchpad link (one per card, pulled from the body)

Tight — this is a dashboard, not a wall of text. ~10 cards max unless the user asks for more.

## Step 4 — drill into a specific card if asked

If the user asks "tell me about DSH-009", **read the linked scratchpad locally** rather than via WebFetch.

Cards link to scratchpads like:
```
...
Return the synthesised dashboard or card detail directly. No parsing notes, no raw kanban excerpts, no apology text — the parent only sees your final message, so make it the thing the user wanted.
